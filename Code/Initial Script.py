...
vac_df.head()
vac_df.info()
vac_df[(vac_df["vaccines"].str.contains("Johnson")) & (vac_df["people_fully_vaccinated"].notna()) & (vac_df["total_vaccinations"].isna())].count()
class VAC:
    def __init__(self, vac_df):
        ###continue clean up
        ...
        ###mean, min, and max weeks?
        vac_summ_stats2 = vac_summ_stats['Weeks For Analysis'].describe()
...
class CASE:
    def __init__(self, cases_df):
        ...
        cases_coverage = cases_df.groupby('location').count()
...
###test cases
class HOFTestCase(unittest.TestCase): 
    
       def test_HOF_country(self):
           os.chdir('C:/Users/MSachs.MSACHS-DELL/Documents/UVA MSDS/CS 5010/Semester Project')

           ###read hofstede data into pandas dataframe
           hof_df2 = pandas.read_csv('Hofstede.csv', 
                        header=0,na_values=('#NULL!',''))
           hof_df2 = HOF(hof_df2)
           counter = hof_df2.hof_df["country"].unique().size
           self.assertEqual(counter, 111)
            
       def test_VAC_country(self):
           os.chdir('C:/Users/MSachs.MSACHS-DELL/Documents/UVA MSDS/CS 5010/Semester Project')
           vac_df = pandas.read_csv('country_vaccinations.csv', 
            header=0,na_values=('#NULL!',''))
           vac_df2 = VAC(vac_df)
           counter = vac_df2.vac_df["Country"].unique().size
           self.assertEqual(counter, 161)
           
       def test_ECON_country(self):
           os.chdir('C:/Users/MSachs.MSACHS-DELL/Documents/UVA MSDS/CS 5010/Semester Project')
           econ_df = pandas.read_csv('microeconomic.csv', 
            header=2,na_values=('#NULL!',''))
           econ_df2 = ECON(econ_df)
           counter = econ_df2.econ_df.reset_index()
           counter = counter["Country Name"].unique().size
           self.assertEqual(counter, 264)
           
       def test_CASE_country(self):
            os.chdir('C:/Users/MSachs.MSACHS-DELL/Documents/UVA MSDS/CS 5010/Semester Project')
            cases_df = pandas.read_csv('owid-covid-data.csv', 
             header=0,na_values=('#NULL!','','nan'))
            cases_df2 = CASE(cases_df)
            counter = cases_df2.cases_df["location"].unique().size
            self.assertEqual(counter, 215)
